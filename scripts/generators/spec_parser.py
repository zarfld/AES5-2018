...
ID_PATTERN = re.compile(r'^(?P<id>(StR|REQ|ARC|ADR|QA|TEST)-(?:[A-Z]{2,4}-)?[A-Z0-9][A-Z0-9\-]*)\b')
# Capture full identifiers with optional 2-4 char category prefix
# Examples: REQ-AUTH-F-001, StR-CORE-001, ADR-INFRA-001, QA-SC-001, TEST-LOGIN-001
REF_PATTERN = re.compile(r'\b(?:StR|REQ|ARC|ADR|QA|TEST)-(?:[A-Z]{2,4}-)?[A-Z0-9][A-Z0-9\-]*\b')
